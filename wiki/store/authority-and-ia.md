...
秩序商店的信息架构分为凝结核内部治理、Store 平台 workflow、Identity Registry 身份解析和 StateMachine 协议事实。Store 给凝结核提供舞台，给 operator 提供目录和审核工具；协议事实来自 Identity Registry 与 StateMachine 事件。
...
| Zhixu draft/version | 凝结核设计，publisher 签名发布 | `PlanCommitted` / `PlanFinalized` 决定链上版本是否可用。 | draft、compile preview、fairness material、platform tags、active recommendation。 |
| Supplier | Store 组织并线下核验 | Identity binding 决定默认名称解析；不证明能力。 | profile、capability tags、联系人、通知渠道、审核状态、历史参与记录。 |
...
- Trust visible：任何 plan/supplier identity 都要显示来自 registry projection 的状态。
...
- Revoked visible but blocked：revoked plan/supplier 可以被 operator 查到，并在创建新订单入口阻断。
...
搜索可以横跨对象，但对象详情页必须显示自己的权威来源。比如 supplier 搜索命中的是 Store profile，trust badge 必须来自 registry projection；Zhixu 卡片可以展示 Store 推荐版本，但 official 状态必须来自 plan publication。

## 边界禁区

- Store 列表排序、平台标签或运营推荐使用 workflow/metadata 口径。
- Store admin 属于平台 workflow 角色；秩序内部治理者是凝结核。
- 不让普通用户页面承担 Store search / governance / supplier directory 职能。
...
- contact delivered、message read、operator note 属于运营记录；业务 signal 看 state-machine event。
