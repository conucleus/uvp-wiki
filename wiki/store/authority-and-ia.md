...
秩序商店的信息架构必须分清三件事：凝结核内部治理、Store 平台 workflow、trust domain 外部背书。Store 给凝结核提供舞台，给 operator 提供目录和审核工具，给 trust reviewer 提供材料和 proof；协议事实来自 registry/state-machine 事件。
...
- Store admin 写成平台 workflow 角色；秩序内部治理者是凝结核。
...
- contact delivered、message read、operator note 写成运营记录；业务 signal 看 state-machine event。
