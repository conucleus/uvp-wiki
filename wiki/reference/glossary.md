...
| OnchainHookPlan | 面向 EVM 注册和背书的紧凑链上产物。 | `OnchainHookPlanArtifact`、compact hooks、dependency indexes、selector bindings。 |
...
| 术语 | 普通解释 | 代码或链上对应物 |
| --- | --- | --- |
| Attestation / 背书 | trust registry 对 plan 或 supplier 作出的可信声明。 | `PlanAttested`、`SupplierAttested`。 |
...
| Registry Boundary | 一个 `ZhixuTrustRegistry` 合约地址就是一个信任边界。产品/Store 可配置自己信任哪个 registry，但 `UVPStateMachine.registerPlan()` 不读取 registry。 | `registryAddress`、`ZhixuTrustRegistry.owner()`。 |
...
| Store | 面向凝结核、Supplier、trust registry、operator 和 proof 视图的产品工作台。 | `zhixu-store/app`、Store Console API。 |
