...
Chain Services 是普通 Node 服务包：`@uvp-eth/chain-services`。它可以作为官方测试网服务运行，也可以被外部团队下载到自己的机器、CI、私有云或集成环境中运行。dev/rebuild 等运行命令见[运行服务和前端](../../how-to/run-services-and-apps.md)。
...
| [Store Console、Supplier 与 Governance API](store-api.md) | `src/store-console/`、`src/store-suppliers/`、`src/governance/` | 凝结核工作台、supplier directory、review、identity binding register/revoke request。 |
| [Notifications 与 Reconcile](notifications-reconcile.md) | `src/notifications/`、`src/reconcile/` | delivery intent、retry/dead-letter、submission/projection reconciliation。 |
| [Storage、Migration 与 Runtime Profile](storage-runtime.md) | `src/storage/`、`src/config/`、`migrations/` | memory/SQLite/PostgreSQL、migration、testnet fail-closed profile。 |
| [API Routes](api-routes.md) | `src/api/routes/` | Product、Store、governance、notification、evidence、diagnostics route module 归属。 |

## 子系统地图

| 子系统 | 输入 | 输出 |
| --- | --- | --- |
| Indexer | deployment registry、state-machine、Identity Registry logs。 | `OrderRegistered`、`SignalSubmitted`、`HookStatusChanged`、`HookReady` 等事件的归一化投影与 sync status。 |
...
| Stage patches | selector-signed executor/resource patch typed data。 | `StageExecutorPatchApplied`、`StageResourcePatchApplied` 的提交路径。 |
| Proof verifier | metadata hash、evidence hash、Zhixu hash、object handle。 | proof mismatch report 与 evidence metadata。 |
| Product projection | chain projection、identity projection、evidence metadata。 | order/task/timeline/proof DTO 与 staging readiness。 |
| Product BFF | order draft、invite、participant wallet、plan publication。 | order registration submit workflow 与 authorization table。 |
| Store Console | Store draft、version、docking、audit、runtime metadata。 | 凝结核工作台与 review 材料。 |
| Store Supplier | supplier metadata、capability tags、contact material。 | 链下 supplier directory 与 review 记录；身份登记请求另行处理。 |
| Identity workflow | Store identity review、admin action。 | `IdentityBindingRegistered` / `IdentityBindingRevoked` 的 tx intent 与 audit。 |
| Notifications | `HookReady`、submitter authorization、supplier identity、profile。 | delivery intents、retry/dead-letter 运行状态。 |
| Reconcile | submission state、chain confirmation、projection lag。 | 后台 reconciliation status 与修复循环。 |
...
- 合约和链事件是唯一事实源；Chain Services 只做投影、中继和展示，全部协议边界以[协议边界总纲](../protocol-boundaries.md)为准。
- Chain Services 是可替换、可 fork 的链下执行软件，负责投影和中继。
- Relayer 可以代付 gas 并广播交易，但不能生成业务签名。
