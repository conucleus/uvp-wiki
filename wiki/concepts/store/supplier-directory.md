...
当 supplier 的 `supplierType=zhixu` 时，它表示一条可被其他秩序调用的 peer 秩序能力：Store 按定义 name 展示 peer Zhixu、版本化 Plan、发布的具名接口（`orderModes` 与端口），以及调用方的 `inputMap`/`signalMap` 映射。docking proof 以 `DockOpened`、`DockInputSubmitted`、`DockOutputSubmitted` 等事件和两边订单的 signal/proof 表达；不能依赖旧的私有 Docked 事件名。local order 必须在 order registration、后续授权路径或 docking 事件路径中明确允许 mapped signal 推进。接入流程与字段细节见 [Zhixu 作为执行接口](../apps/zhixu-as-executor.md)。
