...
- `uvp-executor chain-once`：单轮扫描 `HookReady` 并生成 job 或 dry-run；要点 `--rpc-url`、`--config`、`--dry-run`（`--dry-run` 是显式测试辅助：只演练扫描与准备，不提交任何交易）。被扫描的 state machine 在 `--state-machine` 与 config `stateMachines[]` 中二选一声明，同时给出会被拒绝。
...
- 重试判定先认显式错误码；没有错误码的失败再按已知错误文本归类——RPC/网络类（超时、429/502/503/504、断连、fetch failed）、gas 不足（insufficient funds）与 nonce 冲突判为可重试，重复 signal（`SignalAlreadyExists`）与未授权判为不可重试；无法识别的失败保守地按不可重试处理，不做猜测。
