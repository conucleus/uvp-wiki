...
Store 在链下维护 supplier 的显示名称、联系方式、能力标签、适用角色与阶段、私有匹配特征、审核和推荐记录。这些数据表达 Store 自己的经营判断，不是 UVP 合约认证。

`UVPIdentityRegistry` 可以在完成线下身份核验后，把 supplier 的 `subjectId` 绑定到一个或多个账户。它回答“这个账户在本 Store 目录中代表谁”，不证明能力、信誉、适配性或履约结果。
...
一条 Zhixu 需要对接另一条 Zhixu 时，使用 docking 的 order link、signal mapping 和 proof 机制，不把另一条 Zhixu 注册成 Supplier DSL 对象。
