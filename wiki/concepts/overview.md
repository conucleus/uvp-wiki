...
`uvp-eth` 的核心不是一个后端订单系统，而是一套把 Zhixu 协作定义落到 EVM 链上的状态机协议。合约和链事件是事实源；服务、Store、Order App、executor-kit 都只是围绕这个事实源做编译、索引、展示、签名和提交。
...
先读这些基础词。它们是同一层级的核心概念，不要把它们混成“一个订单对象”：

| 子页 | 说明 |
| --- | --- |
| [Zhixu DSL](core/zhixu.md) | 静态秩序定义，描述 task、stage、source、signal、supplier 边界和选择权。 |
...
| [信任与授权](trust-and-authorization.md) | trust domain、计划认证、订单级 signal 授权、EIP-712 和 relayer 边界。 |
...
- Store、Order App、Product API 可以组织用户语言和元数据，但不能绕过链上授权。
- Store 是一级 trust/workflow 系统，不应塞进普通产品表面。
- Store admin 不是凝结核，也不是 trust domain；它只拥有平台 workflow 权限。
- Executor Kit 是执行者集成面，不是状态机组件，也不是可信后端。
- USDC、escrow、guarantee、AI agent 等都属于 adapter 或 periphery，不能改写协议核心。
