...
从一份 Zhixu 到一笔可运行的 Order，可以拆成四个动作。

### 5.1 先写“这类项目怎么协作”

凝结核先把光伏项目交付的通用协作方式写成 Zhixu。在这个故事里，凝结核可以先理解成拥有这套可复用运营模型的团队或组织，例如采购运营团队、行业项目组织者，或平台侧 workflow 设计者。它回答普通业务问题：谁先开始，EPC 等什么，OEM 什么时候能发货，清关完成后谁收到通知，现场签收后谁可以验收，失败或超时时走哪条路。这一步还不是某个具体项目，只是一类项目的运行规则。

### 5.2 再把规则固化成一个版本

系统检查并打包这份 Zhixu：引用完整、阶段和 signal 能对上之后，形成一个稳定版本和它的指纹。同一份规则永远得到同一个指纹；影响协作语义的改动会得到新指纹。后面审查、注册订单、追责时，大家讨论的都是同一个版本。
...
Order 是某个稳定规则版本的一次具体执行。比如某个蒙古光伏项目真的要采购一批组件并交付到现场，由订单创建者签名 trigger typed data 创建 Order（没有 registrar allowlist），并写入本订单里谁能提交哪些 signal。
...
这四步在链上如何逐步落地（两步 Plan 注册定稿、身份登记、trigger 与授权写入），见[本地到链上路径](../concepts/lifecycle.md)。
...
放回这个光伏 Order，有一对概念要分开：负责进口清关的报关服务商是 [Supplier](../concepts/core/supplier.md)——能力和 trust 主体，接受材料审核；它在项目中实际提交清关完成 signal 的操作钱包、员工钱包或 API 钱包，则是这个阶段的 [Executor](../concepts/core/executor.md)。
...
任务和 proof row 的字段如何从链事件投影而来，见 [Product DTO](../concepts/product/dto.md)。
