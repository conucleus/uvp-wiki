...
| On-chain execution | `uvp-protocol/contracts/uvp-contracts/` | ABI, events, EIP-712 domain, `UVPStateMachine`, modules, and `UVPIdentityRegistry`. |
...
| Rebuildable service layer | `uvp-chain-services/service/` | forkable off-chain indexer, projection, relayer boundary, proof verifier, Product/Store API. |
...
- Contracts and chain events are the source of truth for plan, order, signal, hook, publication, and deployment cutover.
