...
The information architecture of the Store must separate three things: internal Nucleation governance, Store platform workflow, and external trust-domain endorsement. The Store gives Nucleation a stage, gives operators a catalog and review tools, and gives trust reviewers materials and proof. Protocol facts come from registry/state-machine events.

## Information Objects

| Store object | Primary owner | On-chain fact | Store-organized information |
| --- | --- | --- | --- |
| Nucleation | Zhixu designer / Zhixu organizer | No independent contract state; it is reflected in Zhixu/Plan/Order/proof. | identity, maintenance notes, version history, design materials, and publishing materials. |
| Zhixu draft/version | Nucleation design, trust-domain endorsement | `PlanAttested` / `PlanRevoked` determine official trust. | draft, compile preview, fairness materials, platform tags, active recommendation. |
| Supplier | Nucleation organization, trust-domain endorsement | `SupplierAttested` / `SupplierRevoked` determine the trust projection. | profile, capability tags, contacts, notification channels, review status, historical participation records. |
...
- Search first: the same query can hit Nucleation, Zhixu, Order, Supplier, and governance objects.
- Nucleation visible: Zhixu versions should show who designed and maintains them, and also show Store status.
...
- Do not describe Store admin as the platform workflow role; the internal governance owner is Nucleation.
