...
If this docking stage is opened directly by an out-of-order Product/registrar workflow rather than waiting for the previous business signal, declare the link-stage entrance as `externalSignals`:
...
`LINK_READY` only opens the local docking workflow. The linked order's start, completion, or failure still returns to the local order through `signalMap`, docking links, proof checks, and authorized mapped signals.
...
The compiler generates a `kind=signalMap` hook for each `signalMap` (`trigger=false`, so it does not directly become a Product task), validates that expressions parse under hook-core, and validates that referenced local stages/signals exist; one signalMap must reference a single source.

## Docked runtime path

```text
a trigger hook on the local order becomes Ready
...
| Who opened the link stage's external entrance? | The backend/executor's verification, deduplication, and normalization record for `LINK_READY`, or the previous business signal's proof. |
...
| How does the local order continue? | The mapped signal on the local order, from either an authorized submitter or `DockedSignalSubmitted`. |
...
choose local stage
  ...
  -> validate that signalMap matches source/signal
