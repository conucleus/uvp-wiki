...
| Docking relation | `DockedOrderLinked`, `DockedSignalMapped`, `DockedSignalSubmitted`, plus each side's own signal/proof. |
...
If a stage is carried by another Zhixu, a signal binding between the local order and the linked order usually forms: `linkDockedOrder` records the docking relation and signal binding, and once linked proof validation passes, `submitDockedSignal` or an authorized submitter maps the linked signal into the local order. For the full runtime path see [Zhixu as Executor](../apps/zhixu-as-executor.md); Store/Product only keep sandbox, contact, review, and display state — runtime proof follows on-chain events of both orders.
