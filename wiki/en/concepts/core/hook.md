...
A Hook is the smallest state-machine rule used to decide whether a stage condition has been satisfied. It evaluates Order events and may later open a task through a [Trigger](trigger.md). Each item in a Zhixu stage’s `receiveSignals` usually compiles into a `kind=receive` hook; if a stage docks with another Zhixu, `signalMap` also compiles into `kind=signalMap` hooks.
