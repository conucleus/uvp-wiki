...
该入口部署 `UVPStateMachine`、六个冻结 modules、`UVPDeploymentRegistry` 和 `UVPIdentityRegistry`，登记 deployment，并写入地址清单（`uvp-eth.addresses.v1`）。它是 contract-only 入口：到此为止，不注册 Plan——plan/order/dock smoke 由下面的 bootstrap 入口继续。
