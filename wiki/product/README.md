...
产品与执行面把链上事实翻译成 ordinary Product UI、participant task、Product DTO/API 和 signal container。它不创造协议事实。Product API 由 [非可信执行层：Chain Services](../components/chain-services.md) 提供，但本页只讲 Product 语言和用户/执行者消费方式。Store 和执行者集成都已经是单独一级目录；本页只在需要说明 DTO/API 消费方时链接它们。
...
| 页面 | 作用 |
| --- | --- |
| [产品表面](../concepts/product-surfaces.md) | Product 层为何不是普通后台。 |
...
- Store metadata、draft、supplier profile、audit 和 JWT session 属于 [秩序商店](../store/README.md) 语境，是凝结核工作台或平台 workflow 状态，不是协议事实源。
- Product API 可以准备 typed data、验证签名、调用 relayer、返回 proof，但不能绕过 `UVPStateMachine` 授权。
- `uvp-order-app` 已是独立 participant app 边界，但尚未被同一条 Base Sepolia Product API 真实任务流完整证明。
- executor-kit 的 Product API signal producer 和 thin MCP adapter 属于 [执行者与集成](../execution/README.md)，生产运行、密钥治理和 live operator runbook 不能混成协议核心能力。
